  ...
  local function Check()
        ...
        tmp_guid="tmp_guid"
        last_guid=reaper.GetExtState(tmp_guid,"tmp_guid_cycle",false)
        
        if not reaper.HasExtState(tmp_guid,"tmp_guid_cycle") or last_guid=="" then 
           ...
           reaper.SetExtState(tmp_guid,"tmp_guid_cycle",current_guid,false)
  ...
  local function main()
      ...
      if starttime == endtime then -- no loop
         if m then
            reaper.GetSet_LoopTimeRange2(0, true, false, m, M, false)
         else
            Cmd=reaper.NamedCommandLookup("_SWS_RESTLOOP1")
            reaper.Main_OnCommand(Cmd,0)
            SSS, EEE = reaper.GetSet_LoopTimeRange2(0, false, false, 0, 0, false)
            if SSS == EEE then
               if reaper.GetSelectedMediaItem(0,0) then
                  reaper.Main_OnCommand(41039,0)--set time selection to item
                  SSS, EEE = reaper.GetSet_LoopTimeRange2(0, false, false, 0, 0, false)
                  if SSS ~= EEE then
                     if 0 == reaper.GetToggleCommandState(1068) then
                        reaper.Main_OnCommand(1068,0)
                     end
                     cmd=reaper.NamedCommandLookup("_SWS_SAVELOOP1")
                     reaper.Main_OnCommand(cmd,0)
                  end
               end
               return
