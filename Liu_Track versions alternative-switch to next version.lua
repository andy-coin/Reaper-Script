  ...
  local function Rename(tr_a,idx_a)
      ...
      tr_b=reaper.GetTrack(0,idx_a-2)
      ...
      if sub_1 and sub_2 and key_1==key_2 then
  ...
  local function KillSame(tr_A,idx_A)
      ...
      tr_B=reaper.GetTrack(0,idx_A-2)
      _,mane=reaper.GetTrackName(tr_B)
      ...
      if chunk_A==chunk_B and match then 
         if reaper.GetMediaTrackInfo_Value(tr_A,"I_FOLDERDEPTH")==-1 then
            reaper.SetMediaTrackInfo_Value(tr_B,"I_FOLDERDEPTH",-1)
         end
         reaper.DeleteTrack(tr_A)
         reaper.GetSetMediaTrackInfo_String(tr_B,"P_TCP_LAYOUT","d5 -- Tmp",true) 
         return true
      end
  ...
  local function Down()
      ...
      if layout=="d3 ------ Red Fader" then
         ...
         if mom and last_tr then 
            if last_tr==bottom_tr then goto END end
            last_idx=reaper.GetMediaTrackInfo_Value(last_tr,"IP_TRACKNUMBER")
            target_tr=reaper.GetTrack(0,last_idx)
            reaper.GetSetMediaTrackInfo_String(last_tr,"P_TCP_LAYOUT","",true)
            reaper.GetSetMediaTrackInfo_String(target_tr,"P_TCP_LAYOUT","d5 -- Tmp",true)
         elseif mom and not last_tr then
            mom_idx=reaper.GetMediaTrackInfo_Value(mom,"IP_TRACKNUMBER")
            target_tr=reaper.GetTrack(0,mom_idx)
            reaper.GetSetMediaTrackInfo_String(target_tr,"P_TCP_LAYOUT","d5 -- Tmp",true)
         elseif not mom then
            target_tr=reaper.GetTrack(0,cur_idx)
            reaper.GetSetMediaTrackInfo_String(target_tr,"P_TCP_LAYOUT","d1 -- Fader",true)
         end
      ...
      KillSame(target_tr,target_idx)
      --Rename(target_tr,target_idx,mane)
      ::END::
  ...
  Down()
  reaper.Undo_EndBlock("Switch to next track version",-1)
