...
local function Main ()
  ...
  if midiEditor ~= nil then
      ...
      if take ~= nil then
          ...
          --reaper.ShowConsoleMsg(visibleLanes)
          if #visibleLanes == 1 and visibleLanes[1].height <= 9 then
            ...
            cmd=reaper.NamedCommandLookup("_S&M_HIDECCLANES_ME")
            reaper.MIDIEditor_LastFocused_OnCommand(cmd, false)-- SWS/S&M: Hide all CC lanes
          end
      end
  end
  reaper.Undo_EndBlock("Show notes Velocity",-1)
end
